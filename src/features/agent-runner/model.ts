...
const CHAT_HISTORY_KEY = 'agent.chatHistory'
const MAX_HISTORY_THREADS = 20

function loadChatHistory(): ChatHistoryThread[] {
  try {
...
function saveChatHistory(threads: ChatHistoryThread[]) {
  try {
    localStorage.setItem(CHAT_HISTORY_KEY, JSON.stringify(threads.slice(0, MAX_HISTORY_THREADS)))
  } catch {
    /* history is a convenience; the task itself still runs */
  }
}
...
export function useAgentRunner() {
  ...
  const [history, setHistory] = useState<ChatHistoryThread[]>(loadChatHistory)
  ...
  const lastTargetPageRef = useRef<string | null>(null)
  ...
  const archiveCurrentChat = useCallback((reason: 'new-chat' | 'new-page' | 'new-task') => {
    const current = messagesRef.current
    if (current.length === 0) return
    const firstUser = current.find((message) => message.role === 'user')?.text
    const pageUrl = lastTargetPageRef.current || targetPage || ''
    const title = firstUser
      ? firstUser.slice(0, 70)
      : reason === 'new-page'
        ? 'Chat from previous page'
        : 'Previous chat'
    ...
    setHistory((existing) => [thread, ...existing].slice(0, MAX_HISTORY_THREADS))
  }, [targetPage])

  useEffect(() => {
    const previous = lastTargetPageRef.current
    if (previous === null) {
      lastTargetPageRef.current = targetPage
      return
    }
    if (!targetPage || targetPage === previous) return
    if (isRunningRef.current || messagesRef.current.length === 0) {
      lastTargetPageRef.current = targetPage
      return
    }
    archiveCurrentChat('new-page')
    lastTargetPageRef.current = targetPage
    setMessages([])
    messagesRef.current = []
  ...
  }, [archiveCurrentChat, targetPage])

  const startNewChat = useCallback(() => {
    if (isRunningRef.current) return
    archiveCurrentChat('new-chat')
    setError('')
    setMessages([])
    messagesRef.current = []
    seenRef.current = new Set()
